...
namespace System.Security.Cryptography.Asn1
{
    ...
    internal class AsnReader
    {
        ...
        {
            if (TryPeekTag(_data.Span, out var tag, out int bytesRead))
            {
                return tag;
            }
        ...
        {
            if (TryPeekTag(_data.Span, out var tag, out int tagBytesRead) &&
        ...
        private int SeekEndOfContents(ReadOnlyMemory<byte> source)
        {
            ...
            {
                var tag = tmpReader.ReadTagAndLength(out int? length, out int bytesRead);
        ...
        {
            var tag = ReadTagAndLength(out int? length, out int bytesRead);
        ...
        {
            var tag = ReadTagAndLength(out int? length, out int headerLength);
        ...
        {
            var tag = ReadTagAndLength(out int? length, out headerLength);
            ...
            out long value)
        {
            ...
            var contents = GetIntegerContents(expectedTag, tagNumber, out int headerLength);
            ...
            do
            {
                while (tmpReader.HasData)
                {
                    ...
                    else if (tag == Asn1Tag.EndOfContents && isIndefinite)
                    {
                        ...
                        {
                            (var topReader, bool wasIndefinite, int pushedBytesRead) = readerStack.Pop();
